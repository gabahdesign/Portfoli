...
export function MoveAuthModal({ isOpen, onClose, onSuccess }: MoveAuthModalProps) {
  ...
  return (
    <Drawer.Root open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <Drawer.Portal>
        <Drawer.Overlay className="fixed inset-0 bg-black/80 backdrop-blur-md z-[6000]" />
        <Drawer.Content className="bg-[var(--color-surface)] flex flex-col rounded-t-[2.5rem] h-[92%] sm:h-auto sm:max-w-md fixed bottom-0 left-0 right-0 sm:left-1/2 sm:-translate-x-1/2 sm:bottom-1/2 sm:translate-y-1/2 sm:rounded-[2.5rem] z-[6001] outline-none border-t border-white/5 sm:border sm:border-white/10">
          ...
          <div className="flex-1 overflow-y-auto no-scrollbar">
            ...
            <div className="px-8 pb-10">
              ...
              {view === "register" && (
                <form onSubmit={handleInitiateRegister} className="space-y-4 pr-1">
